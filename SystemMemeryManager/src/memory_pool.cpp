...
void init_memory_pool() {
    // 初始化第一个块（覆盖整个内存池）
    MemoryBlock *first_block = reinterpret_cast<MemoryBlock*>(memory_pool.pool);
...
void* pool_alloc(size_t size) {
    ...
    while (current != nullptr) {
        if (current->is_free && current->size >= total_size) {
            ...
            if (current->size >= total_size + sizeof(MemoryBlock) + 16) {
                // 创建新块
                MemoryBlock *new_block = reinterpret_cast<MemoryBlock*>(reinterpret_cast<char*>(current) + total_size);
    ...
    // 没有足够空间，使用标准分配
    std::cout << "Out of memory in pool, malloc size " << size << std::endl;
...
void pool_free(void *ptr) {
    ...
    // 检查指针是否在内存池范围内
    char *pool_start = &memory_pool.pool[0];
    char *pool_end = pool_start + POOL_SIZE;

    if (reinterpret_cast<char*>(block) >= pool_start && reinterpret_cast<char*>(block) < pool_end) {
        // 标记为空闲
        block->is_free = 1;
